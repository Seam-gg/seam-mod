...
object ContainerSweep {
     ...
     * input as project stock. The webapp refuses to *create* a tag on a furnace, but nothing stops
     * the block at a tagged position changing later, so the sweep holds the same line — see
     * [ContainerKind], which the tagging gesture shares.
     *
    ...
    private fun inventoryAt(world: ServerWorld, pos: BlockPos): Inventory? {
        ...
        if (!ContainerKind.isTaggable(block)) return null
        // `false` would ignore a blocked chest (an ocelot or a solid block above it). The stock is
        // in there either way, so read it regardless of whether a player could open it.
        if (block is ChestBlock) return ChestBlock.getInventory(block, state, world, pos, true)
